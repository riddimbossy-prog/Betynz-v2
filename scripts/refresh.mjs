...
export async function refresh({sporty=new Sportybet(),football=new Football(),dataDir='data',today=day(),days=Number(process.env.BOARD_DAYS||1)}={}) {
  ...
  for(const date of dates) {
    ...
    const results=await mapLimit(fixtures,2,async f=>{
      ...
      try {
        ...
        if(policy.blockedLeaguePatterns.some(p=>new RegExp(p,'i').test(f.league.name)))return skipped('Excluded competition type');
        const matched=matchFixture(f,statsByDay.get(date)||[],aliases);
    ...
    const board={version:7,date,generatedAt:new Date().toISOString(),oddsSource:'Sportybet',statisticsSource:'API-Football',
      status:books.complete?'ready':fixtures.length?'partial':'unavailable',complete:books.complete,leagueCount,busyDay:leagueCount>=policy.busyDayLeagueCount,
  ...
  const index={version:7,generatedAt:new Date().toISOString(),startedAt:start,dates:daily,diagnostics,complete:books.complete,policy};
