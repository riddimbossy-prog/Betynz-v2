...
function choosePrimaryMarket(over25, btts, winDnb) {
const candidates = [];
...
score: btts.score,
market: btts.market,
protectionPriority: 1, // BTTS is treated as the riskiest of the qualifying markets.
});
}
...
throw new Error("analyseMatch requires a match object.");
}
...
const home = calculateSplitStats(
input.homeLast5,
`${homeTeam} last 5 HOME matches`
);
const away = calculateSplitStats(
input.awayLast5,
`${awayTeam} last 5 AWAY matches`
);
...
const primary = choosePrimaryMarket(over25, btts, winDnb);
...
: primary.type === "BTTS"
? btts.reasons
: primary.type === "WIN_DNB"
? winDnb.reasons
: ["No market cleared the Golden Banker qualification gates."];
return {
...
throw new Error("selectTopBankers expects an array of analysed matches.");
}
return analyses
.filter((a) => a && a.banker)
.sort((a, b) => {
const scoreDiff =
b.finalRecommendation.score - a.finalRecommendation.score;
...
return (
priority(b.finalRecommendation.primaryBet) -
priority(a.finalRecommendation.primaryBet)
);
})
...
throw new Error("analyseBoard requires an array of matches.");
}
...
philosophy:
"Protect the stake first. Void is better than loss. Last 5 split home/away data only.",
