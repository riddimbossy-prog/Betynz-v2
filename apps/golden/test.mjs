...
assert.match(html,/muted playsinline webkit-playsinline/);
assert.doesNotMatch(html,/autoplay muted playsinline/);
assert.doesNotMatch(html,/controls(?:\s|=)/);
...
assert.match(runtimeJobs,/ANALYSIS_LOCK_REVISION='trust-proof-zeus-v1'/);
...
console.log('Golden engine + trust/proof/Zeus supervisor + settlement + UI + video-only splash tests passed');
