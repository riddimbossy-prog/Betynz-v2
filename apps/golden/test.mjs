...
assert.match(html,/id="splashSharpen"/);
assert.match(html,/feConvolveMatrix/);
...
assert.match(splashCss,/filter:url\(#splashSharpen\)/);
assert.match(splashCss,/\.board-splash__video\.is-portrait/);
assert.match(splashCss,/\.board-splash__video\.is-landscape/);
assert.match(splashCss,/width:min\(88vw,360px\)/);
assert.doesNotMatch(splashCss,/\.board-splash__video\{[^}]*width:100%;height:100%/);
...
assert.match(splashJs,/video\.videoHeight>video\.videoWidth/);
assert.match(splashJs,/classList\.toggle\('is-portrait'/);
...
console.log('Golden Banker v4.3 engine + compact UI + crest proxy + highly rated page + sharp full-duration Zeus splash tests passed');