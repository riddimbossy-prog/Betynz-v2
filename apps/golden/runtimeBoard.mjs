...
const require=createRequire(import.meta.url),{analyseMatch,selectTopBankers,CONFIG}=require('./goldenBanker.cjs');
...
export function marketCode(a){const b=String(a?.finalRecommendation?.primaryBet||'');if(b==='Over 2.5')return'OVER_2_5';if(b==='BTTS Yes')return'BTTS_YES';if(/ DNB$/.test(b))return a?.markets?.winDnb?.favouriteSide==='Home'?'HOME_DNB':'AWAY_DNB';if(/ Win$/.test(b))return a?.markets?.winDnb?.favouriteSide==='Home'?'HOME_WIN':'AWAY_WIN';return'SKIP'}
export function makeBoard(date,board,items,{complete=false,processed=0,total=items.length,warning=null,restored=0}={}){const analyses=items.map(x=>x.analysis).filter(Boolean),top=selectTopBankers(analyses,CONFIG.maxBankers),byId=new Map(items.map(x=>[String(x.fixture?.id||''),x])),topBankers=top.map(a=>{const x=byId.get(String(a.id||''));return x?{fixture:x.fixture,analysis:a}:null}).filter(Boolean);return{engine:'Golden Banker v4.3',engineCode:ENGINE,version:VERSION,date,rules:{sampleSize:5,maxBankers:4,bankerMinScore:7,dnbMinPPG:2,straightWinMinPPG:2.3,opponentPPG:'<1.0',defensiveBleed:'>2.30'},fixtures:(board?.fixtures||[]).filter(f=>!isSrl(f)).map(publicFixture),all:items,topBankers,summary:{fixtures:(board?.fixtures||[]).filter(f=>!isSrl(f)).length,eligible:total,analysed:analyses.filter(a=>!a.waiting).length,waiting:analyses.filter(a=>a.waiting).length,bankersFound:topBankers.length},progress:{processed,total,percent:total?Math.round(processed/total*100):100,restored},complete,warning:warning||board?.warning||null,generatedAt:new Date().toISOString()}}
...
export async function hydrate(date){if(!supabaseConfigured()||!persistenceCoreEnabled())return null;const rows=await loadBoards({date,boardKey:ENGINE,limit:1}).catch(()=>[]),p=rows?.[0]?.payload;if(p?.engineCode===ENGINE){snapshots.set(date,p);return p}return null}
