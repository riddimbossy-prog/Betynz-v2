...
function scheduleRetry(date,force=false){const timer=setTimeout(()=>kick(date,force),ANALYSIS_RETRY_MS);timer.unref?.()}
function cachedFixtureBoard(date,board){if(!board||!Array.isArray(board.fixtures))return null;return{source:'PRELOADED_BOARD',date,cached:true,complete:Boolean(board.complete),generatedAt:board.generatedAt||null,fixtures:board.fixtures}}

async function seedFixtureDate(date){
  let current=snapshots.get(date)||await hydrate(date).catch(()=>null);
  if(current&&Array.isArray(current.fixtures)&&(current.fixtures.length||fixtureSeededDates.has(date))){fixtureSeededDates.add(date);return current}
  const board=await getApiFootballFastFixtureBoard(date),total=(board.fixtures||[]).filter(eligible).length;
  const seed=makeBoard(date,board,[],{complete:false,processed:0,total,warning:board?.warning||null});
  snapshots.set(date,seed);fixtureSeededDates.add(date);
...
async function preloadFixtureLists(){
  ...
  fixtureSeedTask=(async()=>{for(let n=0;n<=PRELOAD_DAYS_AHEAD;n++){const date=utcDate(n);try{await seedFixtureDate(date)}catch{}}})().finally(()=>{fixtureSeedTask=null});
...
async function analyseDate(date,{force=false}={}){
  ...
  const task=(async()=>{
    const lockKey=lockKeyFor(date),lock=await acquireJobLock(lockKey,ANALYSIS_LOCK_LEASE_SECONDS);
    if(!lock.acquired){scheduleRetry(date,force);return snapshots.get(date)||await hydrate(date)}
    let lastRenew=Date.now();
    const renewIfNeeded=async()=>{if(Date.now()-lastRenew<ANALYSIS_LOCK_RENEW_MS)return;await renewJobLock(lockKey,ANALYSIS_LOCK_LEASE_SECONDS).catch(()=>null);lastRenew=Date.now()};
    try{
      const board=await getApiFootballFastFixtureBoard(date),fixtures=(board.fixtures||[]).filter(eligible),saved=force?[]:await loadFixtureStates(date,5000);
      ...
      const items=[];let processed=0,restored=0,warning=null;
      for(const f of fixtures){
        await renewIfNeeded();const id=String(f.id),old=savedMap.get(id);
        if(old){const fixture=publicFixture(f);items.push({fixture,analysis:upgradeAnalysisForCurrentRules(old.payload.analysis,fixture),restored:true});processed++;restored++}
        else{
          try{
        ...
        const snap=makeBoard(date,board,items,{processed,total:fixtures.length,warning,restored});snapshots.set(date,snap);fixtureSeededDates.add(date);
      ...
      const final=makeBoard(date,board,items,{complete:true,processed,total:fixtures.length,warning,restored});snapshots.set(date,final);fixtureSeededDates.add(date);
      ...
      return final;
    }finally{await releaseJobLock(lockKey).catch(()=>null)}
  })().finally(()=>jobs.delete(date));
  jobs.set(date,task);return task;
}
...
export async function settleDate(date){if(!supabaseConfigured())return;const [results,ledger]=await Promise.all([getApiFootballResults(date),getPredictionLedger({from:date,to:date,engine:ENGINE,status:'PENDING',limit:100})]),map=new Map((results.fixtures||[]).map(f=>[String(f.id),f])),updates=[];for(const row of ledger.rows||[]){const f=map.get(String(row.fixture_id)),s=f&&resultScore(f);if(!s)continue;updates.push({...row,settlement_status:settle(String(row.market||''),s.h,s.a),settled_at:new Date().toISOString(),home_score:s.h,away_score:s.a,result_payload:{source:'API_FOOTBALL',score:{home:s.h,away:s.a},lockedAt:row?.payload?.lockedAt||null}})}if(updates.length)await upsertPredictionLedger(updates)}

function proofSummary(rows=[]){
  const settled=rows.filter(r=>['WON','LOST','PUSH'].includes(String(r?.settlement_status||''))),wins=settled.filter(r=>r.settlement_status==='WON').length,losses=settled.filter(r=>r.settlement_status==='LOST').length,pushes=settled.filter(r=>r.settlement_status==='PUSH').length;
  const decided=wins+losses,accuracy=decided?Math.round(wins/decided*1000)/10:null;
  const byMarket={};
  for(const row of settled){const k=String(row.market||'UNKNOWN');byMarket[k]??={won:0,lost:0,push:0,total:0};byMarket[k].total++;if(row.settlement_status==='WON')byMarket[k].won++;else if(row.settlement_status==='LOST')byMarket[k].lost++;else byMarket[k].push++}
  return{published:rows.length,settled:settled.length,pending:rows.filter(r=>r.settlement_status==='PENDING').length,wins,losses,pushes,accuracy,byMarket};
}
...
export async function fixtureBoard(date){if(date>utcDate()){const cached=snapshots.get(date)||await hydrate(date).catch(()=>null),cachedBoard=cachedFixtureBoard(date,cached);if(cachedBoard&&(cachedBoard.fixtures.length||fixtureSeededDates.has(date)))return cachedBoard;try{const seeded=await seedFixtureDate(date),seededBoard=cachedFixtureBoard(date,seeded);if(seededBoard)return seededBoard}catch{}}const b=await getApiFootballFastFixtureBoard(date);return{...b,fixtures:(b.fixtures||[]).filter(f=>!isSrl(f)).map(publicFixture)}}
export const weekCounts=from=>getApiFootballFixtureCounts(from,7),liveBoard=()=>getApiFootballLiveBoard();
export async function proof(from,to){const l=await getPredictionLedger({from,to,engine:ENGINE,limit:500}),rows=l.rows||[];return{from,to,summary:proofSummary(rows),rows}}
...
export async function startRuntime(){for(let n=0;n<=PRELOAD_DAYS_AHEAD;n++){const restored=await hydrate(utcDate(n)).catch(()=>null);if(restored&&Array.isArray(restored.fixtures))fixtureSeededDates.add(utcDate(n))}queueMicrotask(()=>preloadUpcomingWeek().catch(()=>null));const preloadTimer=setInterval(()=>preloadUpcomingWeek().catch(()=>null),PRELOAD_SWEEP_MS);preloadTimer.unref?.();const timer=setInterval(()=>{for(const n of[-2,-1,0])settleDate(utcDate(n)).catch(()=>null)},Math.max(5,Number(process.env.AUTO_SETTLEMENT_INTERVAL_MINUTES||10))*60000);timer.unref?.()}
