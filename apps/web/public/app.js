...
async function loadPrecomputeStatus() {
  ...
  try {
    ...
    const wk = payload.weeklyPrecompute || {};
    const ready = Boolean(payload.visibleWeekReady);
    const preparedDates = Number(payload.preparedViews?.dates?.length || 0);
    const discoveredCounts = Array.isArray(wk.fixtureDiscovery?.counts) ? wk.fixtureDiscovery.counts : [];
    for (const row of discoveredCounts) {
      if (row?.date && row.date !== state.selectedDate) updateWeekCount(row.date, Number(row.count));
    }
    el.classList.toggle('ready', ready);
    el.classList.toggle('working', !ready && Boolean(wk.running));
    const text = ready
      ? `Prepared: all 7 visible days · engines + Consensus + Zeus ready`
      : wk.running
        ? `${wk.fixtureDiscovery?.state === 'READY' ? `Schedule ready (${Number(wk.fixtureDiscovery?.totalFixtures || 0)} fixtures) · ` : 'Discovering week schedule · '}Preparing ${wk.currentDate || wk.from || 'visible week'} · ${Number(wk.processed || 0)}/${Number(wk.total || 7)} engine days complete`
