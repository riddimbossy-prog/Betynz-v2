...
function openValidation(validation){
  ...
  content.innerHTML=`
    ...
    <div class="data-backed-summary"><article><small>Validation score</small><strong>${Number.isFinite(Number(validation.score))?`${Number(validation.score).toFixed(0)}%`:'—'}</strong></article><article><small>Market family</small><strong>${esc(String(validation.family||'MATCH_DATA').replaceAll('_',' '))}</strong></article><article><small>Venue samples</small><strong>${Number(samples.home||0)} + ${Number(samples.away||0)}</strong></article><article><small>Evidence sources</small><strong>${Number(validation.sourceDiversity||0)||'—'}</strong></article></div>
    ${validation.enginePrimarySources?.length?`<div class="independence-meta"><span>${validation.independentlyCorroborated?'Independent corroboration present':'Correlated confirmation only'}</span>${(validation.orthogonalSources||[]).slice(0,3).map(source=>`<span>${esc(String(source).replaceAll('_',' '))}</span>`).join('')}</div>`:''}
