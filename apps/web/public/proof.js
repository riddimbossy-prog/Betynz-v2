...
async function load() {
  ...
  try {
    ...
    $('#proofList').innerHTML = rows.length ? rows.map(row => {
      const engineName = row.engine === 'PPG_ROUTE' ? 'PPG ROUTE' : row.engine === 'APEX_INTELLIGENCE' ? 'APEX INTELLIGENCE' : row.engine === 'CONVERGENCE_ROUTE' ? 'CONVERGENCE' : row.engine === 'MOMENTUM_STREAK' ? 'MOMENTUM & STREAK' : row.engine === 'STREAK_VALUE' ? 'ATLAS STREAK VALUE' : row.engine === 'HTFT_MOMENTUM' ? 'CHRONOS HT/FT' : row.engine === 'ZEUS_SUPERVISOR' ? 'ZEUS SUPERVISOR' : row.engine === 'CONSENSUS_SYSTEM' ? `CONSENSUS · ${String(row.classification||'').replaceAll('_',' ')}` : 'MARKET ROUTE';
      return `<article class="proof-row ${row.recordType === 'CONSENSUS' ? 'consensus-proof-row' : ''}"><div class="proof-main"><span class="proof-status ${String(row.status || 'pending').toLowerCase()}">${esc(row.status)}</span><div><small>${engineName} · ${esc(row.league || 'Competition')}</small><h3>${esc(row.home)} <span>vs</span> ${esc(row.away)}</h3><p>${esc(row.selection || row.market)} @ <b>${Number(row.odds || 0).toFixed(2)}</b> · ${esc(row.decision || 'FIRE')} · Grade ${esc(row.grade || '—')}</p>${dataBackedButton(row.dataValidation,'Backed by data')}</div></div><div class="proof-result"><b>${row.homeScore == null ? '—' : `${row.homeScore}–${row.awayScore}`}</b><span>${fmt(row.kickoff)}</span><small>Frozen ${fmt(row.frozenAt)}</small></div></article>`;
