...
const required=[
  ...
  'src/lib/identityRegistry.mjs','src/lib/featureStore.mjs','src/lib/preparedViews.mjs','src/lib/requestGuard.mjs','src/lib/telemetry.mjs',
  ...
  'public/zeus.html','public/zeus.js','sql/016_zeus_statistical_supervisor.sql','sql/017_foundation_intelligence.sql','sql/018_weekly_precomputed_intelligence.sql','package.json','package-lock.json','.env.example'
];
for(const p of required) await access(resolve(appRoot,p));
for(const p of ['render.yaml','package.json','scripts/verify-single-render.mjs','RELEASE_V5_1_1.md']) await access(resolve(repoRoot,p));
...
for(const token of ['buildPredictionLineage','featureStoreStats','preparedViewStats','precomputeWeek','publicAnalysisDateState','telemetrySnapshot','logPredictionLineage','persistFeatureSnapshots']) if(!server.includes(token)) throw new Error(`Missing foundation wiring ${token}.`);
for(const route of ['streak-value-board','htft-momentum-board','zeus-board','ppg-route-board','apex-intelligence-board','convergence-route-board','momentum-streak-board','precompute-status','admin/precompute-week','admin/telemetry']) if(!server.includes(route)) throw new Error(`Missing route ${route}.`);
...
for(const key of ['FEATURE_STORE_MAX_ENTRIES','WEEKLY_PRECOMPUTE_ENABLED','WEEKLY_PRECOMPUTE_DAYS','PREPARED_VIEW_MAX_ENTRIES','PUBLIC_API_REQUESTS_PER_MINUTE','STATS_API_MAPPING_THRESHOLD']) if(!render.includes(key)||!env.includes(key)) throw new Error(`Foundation env setting missing ${key}.`);
...
if(!/betynz-v5-1-1/.test(sw)||!/engine-lab\.html/.test(sw)||!/api-client\.js/.test(sw)) throw new Error('PWA foundation shell incomplete.');
...
console.log(`Release verification passed: Betynz ${VERSION} Weekly Precomputed Intelligence with persistent prepared views, correlation-aware Consensus, lineage, historical integrity and Zeus supervision; ${tests.length} test files.`);
