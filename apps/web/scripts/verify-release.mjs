...
if (!/const APP_VERSION = '5\.0\.1'/.test(server)) throw new Error('Server version is not 5.0.1.');
for (const token of [
...
if (!/betynz-v5-0-1/.test(sw)) throw new Error('Service-worker cache was not bumped to v5.0.1.');
...
if (pkg.version !== '5.0.1' || lock.version !== '5.0.1' || rootPkg.version !== '5.0.1') throw new Error('Package versions are not 5.0.1.');
...
console.log('Release verification passed: Betynz 5.0.1 API-Football-only single-service release.');
