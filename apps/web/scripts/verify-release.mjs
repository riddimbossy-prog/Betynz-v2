...
const required=[
  ...
  'src/lib/identityRegistry.mjs','src/lib/featureStore.mjs','src/lib/preparedViews.mjs','src/lib/requestGuard.mjs','src/lib/telemetry.mjs','src/lib/persistenceCore.mjs',
  ...
  'public/index.html','public/engine-lab.html','public/app.js','public/api-client.js','public/data-backed-ui.js','public/styles.css','public/sw.js','public/admin-operations.html','public/admin-operations.js',
  'public/zeus.html','public/zeus.js','sql/016_zeus_statistical_supervisor.sql','sql/017_foundation_intelligence.sql','sql/018_weekly_precomputed_intelligence.sql','sql/019_persistence_core.sql','package.json','package-lock.json','.env.example'
];
for(const p of required) await access(resolve(appRoot,p));
for(const p of ['render.yaml','package.json','scripts/verify-single-render.mjs','RELEASE_V5_2_0.md']) await access(resolve(repoRoot,p));
...
for(const token of ['buildPredictionLineage','featureStoreStats','preparedViewStats','precomputeWeek','publicAnalysisDateState','telemetrySnapshot','logPredictionLineage','persistFeatureSnapshots','checkpointFixtureStates','acquireJobLock','persistenceOperationsSnapshot']) if(!server.includes(token)) throw new Error(`Missing foundation wiring ${token}.`);
for(const route of ['streak-value-board','htft-momentum-board','zeus-board','ppg-route-board','apex-intelligence-board','convergence-route-board','momentum-streak-board','precompute-status','admin/precompute-week','admin/telemetry','admin/operations','admin/operations/recompute-fixture']) if(!server.includes(route)) throw new Error(`Missing route ${route}.`);
...
for(const key of ['FEATURE_STORE_MAX_ENTRIES','WEEKLY_PRECOMPUTE_ENABLED','WEEKLY_PRECOMPUTE_DAYS','PREPARED_VIEW_MAX_ENTRIES','PUBLIC_API_REQUESTS_PER_MINUTE','STATS_API_MAPPING_THRESHOLD','PERSISTENCE_CORE_ENABLED','PERSISTENCE_DATE_LOCK_SECONDS','PERSISTENCE_WEEK_LOCK_SECONDS']) if(!render.includes(key)||!env.includes(key)) throw new Error(`Foundation env setting missing ${key}.`);
...
if(!/betynz-v5-2-0/.test(sw)||!/engine-lab\.html/.test(sw)||!/api-client\.js/.test(sw)) throw new Error('PWA foundation shell incomplete.');
...
const weeklySql=await read('sql/018_weekly_precomputed_intelligence.sql');
if(!weeklySql.includes('prepared_intelligence_views')) throw new Error('Weekly prepared-view migration missing.');
const persistenceSql=await read('sql/019_persistence_core.sql');
for(const table of ['prediction_ledger','fixture_processing_state','persistence_job_runs','persistence_job_locks','board_snapshots']) if(!persistenceSql.includes(table)) throw new Error(`Persistence migration missing ${table}.`);
for(const fn of ['betynz_acquire_job_lock','betynz_release_job_lock','betynz_upsert_board_snapshot']) if(!persistenceSql.includes(fn)) throw new Error(`Persistence migration missing ${fn}.`);
...
for(const name of ['foundation-intelligence.test.mjs','universal-odds-gate.test.mjs','runtime-stability.test.mjs','data-backed-validation.test.mjs','adaptive-market-recovery.test.mjs','persistence-core.test.mjs']) if(!tests.includes(name)) throw new Error(`Missing test ${name}.`);
console.log(`Release verification passed: Betynz ${VERSION} Persistence Core with resumable fixture checkpoints, scheduler leases, immutable prediction ledger, prepared views, correlation-aware Consensus and Zeus supervision; ${tests.length} test files.`);
