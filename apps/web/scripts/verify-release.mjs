...
if (!/const APP_VERSION = '5\.0\.0'/.test(server)) throw new Error('Server version is not 5.0.0.');
for (const token of [
...
if (!/betynz-v5-0-0/.test(sw)) throw new Error('Service-worker cache was not bumped to v5.0.0.');
...
if (pkg.version !== '5.0.0' || lock.version !== '5.0.0' || rootPkg.version !== '5.0.0') throw new Error('Package versions are not 5.0.0.');
...
const allowed = [
  ...
  'three-engine-reset.test.mjs','qualified-picks.test.mjs','responsive-cinematic.test.mjs','api-football.test.mjs','api-football-source.test.mjs','platform-smoke.test.mjs'
].sort();
if (JSON.stringify(tests) !== JSON.stringify(allowed)) throw new Error(`Unexpected test files remain: ${tests.join(', ')}`);

console.log('Release verification passed: Betynz 5.0.0 API-Football-only single-service release.');
