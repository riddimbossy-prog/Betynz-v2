...
test('engine HTTP routes return progress immediately while histories continue in background', async t => {
  ...
  started=Date.now(); let ppg=await(await fetch(`http://127.0.0.1:${port}/api/ppg-route-board?date=${date}`)).json(); assert.ok(Date.now()-started<1000); assert.equal(ppg.complete,false); assert.equal(ppg.summary.fixtures,1);
  ...
  const end=Date.now()+8000; while((!ppg.complete||!market.complete)&&Date.now()<end){await pause(250); if(!ppg.complete) ppg=await(await fetch(`http://127.0.0.1:${port}/api/ppg-route-board?date=${date}`)).json(); if(!market.complete) market=await(await fetch(`http://127.0.0.1:${port}/api/market-route-board?date=${date}`)).json();} assert.equal(ppg.complete,true); assert.equal(ppg.failed,false); assert.equal(market.complete,true); assert.equal(market.failed,false);
});
