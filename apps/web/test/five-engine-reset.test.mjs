...
test('production server exposes the five engines with API-Football as the sole provider', async () => {
  const server = await read('src/server.mjs');
  assert.match(server, /MARKET_ROUTE/);
  assert.match(server, /PPG_ROUTE/);
  assert.match(server, /APEX_INTELLIGENCE/);
...
test('public dashboard contains the five current engines and no retired engines', async () => {
  const html = await read('public/index.html');
  assert.match(html, /Market Route/);
  assert.match(html, /PPG Route/);
  assert.match(html, /Apex Intelligence/);
...
test('only the current five engine modules and provider module remain', async () => {
  for (const path of ['src/engines/marketRoute.mjs','src/engines/ppgRoute.mjs','src/engines/apexIntelligence.mjs','src/engines/convergence.mjs','src/engines/momentumStreak.mjs','src/engines/consensus.mjs','src/lib/apiFootball.mjs']) {
