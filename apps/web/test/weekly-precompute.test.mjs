...
test('weekly scheduler discovers fixture counts at high priority and prepares future days before a huge current day', async () => {
  const api = await readFile(new URL('../src/lib/apiFootball.mjs', import.meta.url), 'utf8');
  const server = await readFile(new URL('../src/server.mjs', import.meta.url), 'utf8');
  const app = await readFile(new URL('../public/app.js', import.meta.url), 'utf8');
  assert.match(api, /getApiFootballFixtureCounts[\s\S]*__priority: 0/);
  assert.match(server, /fixtureDiscovery/);
  assert.match(server, /orderedOffsets = from === utcDateOffset\(0\).*\[\.\.\.offsets\.slice\(1\), 0\]/);
  assert.match(server, /await getApiFootballFixtureCounts\(from, days\)/);
  assert.match(app, /fixtureDiscovery\?\.counts/);
  assert.match(app, /Schedule ready/);
});
