...
const weeklyPrecomputeStatus = {
  ...
  lastError: null,
  fixtureDiscovery: { state: 'IDLE', counts: [], totalFixtures: 0, fetchedAt: null, error: null },
  dates: {}
};
...
async function precomputeWeek(from = utcDateOffset(0), { force = false, reason = 'ROLLING_HORIZON' } = {}) {
  ...
  const task = (async () => {
    ...
    try {
      ...
      const counts = Array.isArray(discovered?.counts) ? discovered.counts : [];
      weeklyPrecomputeStatus.fixtureDiscovery = {
        state: 'READY',
        counts,
        totalFixtures: counts.reduce((sum, row) => sum + Math.max(0, Number(row?.count || 0)), 0),
        fetchedAt: discovered?.fetchedAt || new Date().toISOString(),
        error: null
      };
    } catch (error) {
      weeklyPrecomputeStatus.fixtureDiscovery = {
        state: 'ERROR', counts: [], totalFixtures: 0, fetchedAt: null,
        error: error?.message || 'fixture_discovery_failed'
      };
    }
